...
param_scheduler = [
    ...
    dict(type="MultiStepLR", by_epoch=False, begin=0, end=max_iters, milestones=[40], gamma=0.1),
]

optim_wrapper = dict(type="OptimWrapper", optimizer=dict(type="AdamW", lr=1e-4, weight_decay=0.05))

default_hooks = dict(logger=dict(interval=10), checkpoint=dict(interval=max_iters))
