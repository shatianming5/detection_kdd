# checkmd_mmdet_diffusers_stub.md — `check.md` 的 MMDet+Diffusers 口径在本仓库内的对齐说明（已落地）

`check.md` 的 Phase1 明确要求 **PyTorch + MMDetection + Diffusers** 的工程栈（`HybridDiffusionPipeline` + 自定义 collate）。

本仓库主线仍是 **Detectron2/DiffusionDet**；为对齐 `check.md` 的工程栈（MMDet3 + Diffusers + 固定 N 图数据），本仓库新增并补齐：
...
目标：按 `check.md:6.1` 的栈与形态运行（MMDet3 + Diffusers + 固定 N 图 collate）。
...
# 2) 安装 PyTorch 2.1 + CUDA 12.1 wheels（check.md:6.1 口径）
...
如果你的真实目标是 `check.md` 的功能点与实验口径，而非严格 MMDet 同栈，则当前仓库已覆盖：
...
对照与证据入口：`checkmd_parity.md`、`results_manifest.tsv`
